...
const server_1 = __importDefault(require("./lib/interactive/server"));
const client_1 = __importDefault(require("./lib/interactive/client"));
const room_1 = require("./lib/room");
/* eslint-disable no-console */
console.log('process.env.DEBUG:', process.env.DEBUG);
console.log('config.js:\n%s', JSON.stringify(config_1.default, null, '  '));
...
async function runMediasoupWorkers() {
    ...
    for (let i = 0; i < numWorkers; ++i) {
        const worker = await mediasoup.createWorker({
            dtlsCertificateFile: config_1.default.mediasoup.workerSettings.dtlsCertificateFile,
            dtlsPrivateKeyFile: config_1.default.mediasoup.workerSettings.dtlsPrivateKeyFile,
        ...
        if (process.env.MEDIASOUP_USE_WEBRTC_SERVER !== 'false') {
            ...
            // for each Worker.
            const webRtcServerOptions = utils.clone(config_1.default.mediasoup.webRtcServerOptions);
...
async function runHttpsServer() {
    ...
    const tls = {
        cert: fs.readFileSync(config_1.default.https.tls.cert),
        key: fs.readFileSync(config_1.default.https.tls.key)
    };
...
async function runProtooWebSocketServer() {
    ...
    protooWebSocketServer.on('connectionrequest', (info, accept, reject) => {
        ...
        logger.info('protoo connection request [roomId:%s, peerId:%s, address:%s, origin:%s]', roomId, peerId, info.socket.remoteAddress, info.origin);
...
async function getOrCreateRoom({ roomId, consumerReplicas }) {
    ...
    if (!room) {
        ...
        room = await room_1.Room.create({ mediasoupWorker, roomId, consumerReplicas });
