...
const heapdump = __importStar(require("heapdump"));
        ...
        const readStdin = () => {
            cmd.question('cmd> ', async (input) => {
                ...
                switch (command) {
                    ...
                    case 'heapsnapshot':
                        {
                            const filename = `${process.env.SNAPSHOT_DIR || '/tmp'}/${Date.now()}-mediasoup-demo.heapsnapshot`;
                            // eslint-disable-next-line no-shadow
                            heapdump.writeSnapshot(filename, (error, filename) => {
                                if (!error) {
                                    this.log(`heapdump snapshot writen to ${filename}`);
                                    this.log('learn how to use it at https://github.com/bnoordhuis/node-heapdump');
                                }
                                else {
