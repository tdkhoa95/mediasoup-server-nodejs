...
const logger = new logger_1.default('Room');
class Room extends events_1.default {
    ...
    static async create({ mediasoupWorker, roomId, consumerReplicas }) {
        ...
        const bot = await bot_1.Bot.create({ mediasoupRouter });
        return new Room({
            roomId,
            protooRoom,
    ...
    constructor({ roomId, protooRoom, webRtcServer, mediasoupRouter, audioLevelObserver, activeSpeakerObserver, consumerReplicas, bot }) {
        super();
    ...
    logStatus() {
        logger.info('logStatus() [roomId:%s, protoo Peers:%s]', this._roomId, this._protooRoom.peers.length);
    }
    /**
    ...
    handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
        ...
        peer.on('request', (request, accept, reject) => {
            logger.debug('protoo Peer "request" event [method:%s, peerId:%s]', request.method, peer.id);
     ...
     */
    async createBroadcasterTransport({ broadcasterId, type, rtcpMux = false, comedia = true, sctpCapabilities }) {
        const broadcaster = this._broadcasters.get(broadcasterId);
        if (!broadcaster)
            throw new Error(`broadcaster with id "${broadcasterId}" does not exist`);
        switch (type) {
            case 'webrtc':
                {
                    ...
                    const transport = await this._mediasoupRouter.createWebRtcTransport(Object.assign(Object.assign({}, webRtcTransportOptions), { webRtcServer: this._webRtcServer }));
                    // Store it.
    ...
    async createBroadcasterProducer({ broadcasterId, transportId, kind, rtpParameters }) {
        ...
        producer.on('videoorientationchange', (videoOrientation) => {
            logger.debug('broadcaster producer "videoorientationchange" event [producerId:%s, videoOrientation:%o]', producer.id, videoOrientation);
        });
            ...
            case 'createWebRtcTransport':
                {
                    ...
                    const webRtcTransportOptions = Object.assign(Object.assign({}, config_1.default.mediasoup.webRtcTransportOptions), { enableSctp: Boolean(sctpCapabilities), numSctpStreams: (sctpCapabilities || {}).numStreams, appData: { producing, consuming } });
                    if (forceTcp) {
                    ...
                    transport.on('trace', (trace) => {
                        logger.debug('transport "trace" event [transportId:%s, trace.type:%s, trace:%o]', transport.id, trace.type, trace);
            ...
            case 'produce':
                {
                    ...
                    producer.on('videoorientationchange', (videoOrientation) => {
                        logger.debug('producer "videoorientationchange" event [producerId:%s, videoOrientation:%o]', producer.id, videoOrientation);
                    });
                    // NOTE: For testing.
                    ...
                    producer.on('trace', (trace) => {
                        logger.debug('producer "trace" event [producerId:%s, trace.type:%s, trace:%o]', producer.id, trace.type, trace);
                    });
            ...
            case 'applyNetworkThrottle':
                {
                    ...
                    try {
                        ...
                        await throttle_1.default.start({
                            up: uplink || DefaultUplink,
                            down: downlink || DefaultDownlink,
                            rtt: rtt || DefaultRtt,
                            packetLoss: packetLoss || DefaultPacketLoss
                        });
                        logger.warn('network throttle set [uplink:%s, downlink:%s, rtt:%s, packetLoss:%s]', uplink || DefaultUplink, downlink || DefaultDownlink, rtt || DefaultRtt, packetLoss || DefaultPacketLoss);
                        accept();
                    }
                    catch (error) {
        ...
        for (let i = 0; i < consumerCount; i++) {
            promises.push((async () => {
                ...
                consumer.on('trace', (trace) => {
                    logger.debug('consumer "trace" event [producerId:%s, trace.type:%s, trace:%o]', consumer.id, trace.type, trace);
                });
     ...
     * @async
     */
    async _createDataConsumer({ dataConsumerPeer, dataProducerPeer = null, // This is null for the bot DataProducer.
    dataProducer }) {
